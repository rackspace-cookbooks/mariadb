...
describe 'stepped into mariadb_test_custom::server 5.0 on centos-5.8' do
  let(:centos_5_8_custom_run) do
    ChefSpec::Runner.new(
      ...
      node.set['mariadb']['service_name'] = 'centos_5_8_custom'
      node.set['mariadb']['version'] = '5.0'
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    it 'creates mariadb_service[centos_5_8_custom]' do
      expect(centos_5_8_custom_run).to create_mariadb_service('centos_5_8_custom').with(
        :version => '5.0',
        :port => '3308',
        :data_dir => '/data'
        )
    ...
    it 'steps into mariadb_service and installs package[mariadb-server]' do
      expect(centos_5_8_custom_run).to install_package('mariadb-server')
    end

    it 'steps into mariadb_service and creates directory[/etc/mariadb/conf.d]' do
      expect(centos_5_8_custom_run).to create_directory('/etc/mariadb/conf.d').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/run/mariadbd]' do
      expect(centos_5_8_custom_run).to create_directory('/var/run/mariadbd').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates service[mariadbd]' do
      expect(centos_5_8_custom_run).to start_service('mariadbd')
      expect(centos_5_8_custom_run).to enable_service('mariadbd')
    end

    it 'steps into mariadb_service and waits for mariadb to start' do
      expect(centos_5_8_custom_run).to run_execute('wait for mariadb').with(
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb_grants.sql]' do
      expect(centos_5_8_custom_run).to create_template('/etc/mariadb_grants.sql').with(
        :cookbook => 'mariadb',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb_grants.sql]' do
      expect(centos_5_8_custom_run).to render_file('/etc/mariadb_grants.sql').with_content(
    ...
    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(centos_5_8_custom_run).to_not run_bash('move mariadb data to datadir')
    end
