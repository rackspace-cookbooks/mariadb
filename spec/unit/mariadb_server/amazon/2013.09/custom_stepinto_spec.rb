...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and creates directory[/etc/mariadb/conf.d]' do
      expect(amazon_2013_09_custom_run).to create_directory('/etc/mariadb/conf.d').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/run/mariadbd]' do
      expect(amazon_2013_09_custom_run).to create_directory('/var/run/mariadbd').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates service[mariadbd]' do
      expect(amazon_2013_09_custom_run).to start_service('mariadbd')
      expect(amazon_2013_09_custom_run).to enable_service('mariadbd')
    end
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb_grants.sql]' do
      expect(amazon_2013_09_custom_run).to create_template('/etc/mariadb_grants.sql').with(
        :cookbook => 'mariadb',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb_grants.sql]' do
      expect(amazon_2013_09_custom_run).to render_file('/etc/mariadb_grants.sql').with_content(
