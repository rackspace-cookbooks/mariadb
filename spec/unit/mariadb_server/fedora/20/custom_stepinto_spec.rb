...
describe 'stepped into mariadb_test_custom::server on fedora-20' do
  ...
  let(:my_cnf_5_5_content_fedora_20) do
    'This my template. There are many like it but this one is mine.'
  end

  let(:grants_sql_content_custom_fedora_20) do
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and renders file[/etc/my.cnf]' do
      expect(fedora_20_custom_run).to render_file('/etc/my.cnf').with_content(
        my_cnf_5_5_content_fedora_20
        )
    end
    ...
    it 'steps into mariadb_service and runs execute[wait for mysql]' do
      expect(fedora_20_custom_run).to run_execute('wait for mysql')
    end
    ...
    it 'steps into mariadb_service and renders file[/etc/mysql_grants.sql]' do
      expect(fedora_20_custom_run).to render_file('/etc/mysql_grants.sql').with_content(
        grants_sql_content_custom_fedora_20
        )
    end
    ...
    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(fedora_20_custom_run).to_not run_bash('move mariadb data to datadir')
    end
    ...
    it 'steps into mariadb_service and writes log[notify restart]' do
      expect(fedora_20_custom_run).to write_log('notify restart')
    end

    it 'steps into mariadb_service and writes log[notify reload]' do
      expect(fedora_20_custom_run).to write_log('notify reload')
    end
  end
