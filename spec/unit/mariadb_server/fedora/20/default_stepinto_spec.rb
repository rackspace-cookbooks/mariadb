...
describe 'stepped into mariadb_test_default::server on fedora-20' do
  ...
  let(:my_cnf_5_5_content_default_fedora_20) do
    '[client]
port                           = 3306
...
'
  ...
  let(:grants_sql_content_default_fedora_20) do
...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and renders file[/etc/my.cnf]' do
      expect(fedora_20_default_run).to render_file('/etc/my.cnf').with_content(
        my_cnf_5_5_content_default_fedora_20
        )
    end
    ...
    it 'steps into mariadb_service and runs execute[wait for mysql]' do
      expect(fedora_20_default_run).to run_execute('wait for mysql')
    end
    ...
    it 'steps into mariadb_service and renders file[/etc/mysql_grants.sql]' do
      expect(fedora_20_default_run).to render_file('/etc/mysql_grants.sql').with_content(
        grants_sql_content_default_fedora_20
        )
    end
    ...
    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(fedora_20_default_run).to_not run_bash('move mariadb data to datadir')
    end
