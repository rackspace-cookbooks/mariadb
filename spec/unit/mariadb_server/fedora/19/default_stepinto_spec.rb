...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and installs package[community-mariadb-server]' do
      expect(fedora_19_default_run).to install_package('community-mariadb-server')
    end
    ...
    it 'steps into mariadb_service and creates directory[/var/run/mariadbd]' do
      expect(fedora_19_default_run).to create_directory('/var/run/mariadbd').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/lib/mariadb]' do
      expect(fedora_19_default_run).to create_directory('/var/lib/mariadb').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates service[mariadbd]' do
      expect(fedora_19_default_run).to start_service('mariadbd')
      expect(fedora_19_default_run).to enable_service('mariadbd')
    end

    it 'steps into mariadb_service and runs execute[wait for mariadb]' do
      expect(fedora_19_default_run).to run_execute('wait for mariadb')
    end

    it 'steps into mariadb_service and creates template[/etc/mariadb_grants.sql]' do
      expect(fedora_19_default_run).to create_template('/etc/mariadb_grants.sql').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
        )
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb_grants.sql]' do
      expect(fedora_19_default_run).to render_file('/etc/mariadb_grants.sql').with_content(
