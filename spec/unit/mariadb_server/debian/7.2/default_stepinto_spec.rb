...
describe 'stepped into mariadb_test_default::server on debian-7.2' do
...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and creates template[/var/cache/local/preseeding/mariadb-server.seed]' do
      expect(debian_7_2_default_run).to create_template('/var/cache/local/preseeding/mariadb-server.seed').with(
        :cookbook => 'mariadb',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and creates execute[preseed mariadb-server]' do
      expect(debian_7_2_default_run).to_not run_execute('preseed mariadb-server').with(
    ...
    it 'steps into mariadb_service and installs package[mariadb-server-5.5]' do
      expect(debian_7_2_default_run).to install_package('mariadb-server-5.5')
    end

    it 'steps into mariadb_service and creates service[mariadb]' do
      expect(debian_7_2_default_run).to start_service('mariadb')
      expect(debian_7_2_default_run).to enable_service('mariadb')
    end

    it 'steps into mariadb_service and creates directory[/etc/mariadb/conf.d]' do
      expect(debian_7_2_default_run).to create_directory('/etc/mariadb/conf.d').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/run/mariadbd]' do
      expect(debian_7_2_default_run).to create_directory('/var/run/mariadbd').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/lib/mariadb]' do
      expect(debian_7_2_default_run).to create_directory('/var/lib/mariadb').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb_grants.sql]' do
      expect(debian_7_2_default_run).to create_template('/etc/mariadb_grants.sql').with(
        :cookbook => 'mariadb',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb_grants.sql]' do
      expect(debian_7_2_default_run).to render_file('/etc/mariadb_grants.sql').with_content(
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb/debian.cnf]' do
      expect(debian_7_2_default_run).to create_template('/etc/mariadb/debian.cnf').with(
        :cookbook => 'mariadb',
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb/my.cnf]' do
      expect(debian_7_2_default_run).to create_template('/etc/mariadb/my.cnf').with(
        :cookbook => 'mariadb',
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb/my.cnf]' do
      expect(debian_7_2_default_run).to render_file('/etc/mariadb/my.cnf').with_content(
        my_cnf_5_5_content_default_debian_7_2
        )
    end

    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(debian_7_2_default_run).to_not run_bash('move mariadb data to datadir')
    end
  end
end
