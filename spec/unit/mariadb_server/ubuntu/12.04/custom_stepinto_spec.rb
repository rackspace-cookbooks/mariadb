...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    it 'creates mariadb_service[ubuntu_12_04_default]' do
      expect(ubuntu_12_04_custom_run).to create_mariadb_service('ubuntu_12_04_default').with(
        :parsed_version => '5.5',
        :parsed_port => '3308',
    ...
    it 'steps into mariadb_service and creates template[/var/cache/local/preseeding/mariadb-server.seed]' do
      expect(ubuntu_12_04_custom_run).to create_template('/var/cache/local/preseeding/mariadb-server.seed').with(
        :cookbook => 'mariadb',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mariadb_service and creates execute[preseed mariadb-server]' do
      expect(ubuntu_12_04_custom_run).to_not run_execute('preseed mariadb-server').with(
    ...
    it 'steps into mariadb_service and installs package[mariadb-server-5.5]' do
      expect(ubuntu_12_04_custom_run).to install_package('mariadb-server-5.5')
    end
    ...
    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(ubuntu_12_04_custom_run).to_not run_bash('move mariadb data to datadir')
    end
