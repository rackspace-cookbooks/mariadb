...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mariadb_service and creates template[/var/cache/local/preseeding/mariadb-server.seed]' do
      expect(ubuntu_10_04_default_run).to create_template('/var/cache/local/preseeding/mariadb-server.seed').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
        )
    ...
    it 'steps into mariadb_service and creates execute[preseed mariadb-server]' do
      expect(ubuntu_10_04_default_run).to_not run_execute('preseed mariadb-server').with(
    ...
    it 'steps into mariadb_service and installs package[mariadb-server-5.1]' do
      expect(ubuntu_10_04_default_run).to install_package('mariadb-server-5.1')
    end
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb/debian.cnf]' do
      expect(ubuntu_10_04_default_run).to create_template('/etc/mariadb/debian.cnf').with(
        :cookbook => 'mariadb',
    ...
    it 'steps into mariadb_service and creates template[/etc/apparmor.d/usr.sbin.mariadbd]' do
      expect(ubuntu_10_04_default_run).to create_template('/etc/apparmor.d/usr.sbin.mariadbd').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0644'
        )
    ...
    it 'steps into mariadb_service and creates service[apparmor-mariadb]' do
      expect(ubuntu_10_04_default_run).to_not start_service('apparmor-mariadb')
    end

    it 'steps into mariadb_service and creates service[mariadb]' do
      expect(ubuntu_10_04_default_run).to start_service('mariadb')
      expect(ubuntu_10_04_default_run).to enable_service('mariadb')
    end

    it 'steps into mariadb_service and creates directory[/etc/mariadb/conf.d]' do
      expect(ubuntu_10_04_default_run).to create_directory('/etc/mariadb/conf.d').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/run/mariadbd]' do
      expect(ubuntu_10_04_default_run).to create_directory('/var/run/mariadbd').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates directory[/var/lib/mariadb]' do
      expect(ubuntu_10_04_default_run).to create_directory('/var/lib/mariadb').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb_grants.sql]' do
      expect(ubuntu_10_04_default_run).to create_template('/etc/mariadb_grants.sql').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
        )
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb_grants.sql]' do
      expect(ubuntu_10_04_default_run).to render_file('/etc/mariadb_grants.sql').with_content(
    ...
    it 'steps into mariadb_service and creates template[/etc/mariadb/my.cnf]' do
      expect(ubuntu_10_04_default_run).to create_template('/etc/mariadb/my.cnf').with(
        :owner => 'mariadb',
        :group => 'mariadb',
        :mode => '0600'
        )
    ...
    it 'steps into mariadb_service and renders file[/etc/mariadb/my.cnf]' do
      expect(ubuntu_10_04_default_run).to render_file('/etc/mariadb/my.cnf').with_content(
        my_cnf_5_5_content_default_ubuntu_10_04
        )
    end

    it 'steps into mariadb_service and creates bash[move mariadb data to datadir]' do
      expect(ubuntu_10_04_default_run).to_not run_bash('move mariadb data to datadir')
    end
  end
end
